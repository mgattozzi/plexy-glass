...
(see **Project status** below). Per-feature design docs (a spec, and usually a
plan) are kept **local-only** — they are not tracked in the repo, so they won't
...
Plans are local-only (not tracked), task-by-task with full code per step.
Follow the plan; do not invent scope. If a step is wrong, fix the plan first,
then proceed.
...
- **`cargo +nightly fmt --all` before every commit**, and it must pass
  `cargo +nightly fmt --all --check` — this is a real gate, **CI enforces it**.
  The `rustfmt.toml` uses nightly-only options (`imports_granularity`,
  `group_imports`), so plain *stable* `cargo fmt` silently **skips import
  grouping** ("unstable features are only available in nightly channel") and CI
  then fails. Use the `+nightly` toolchain; stable `cargo fmt` is not enough.
...
painter in `crates/mux/src/borders.rs`.
...
Inline graphics (shipped — `docs/inline-graphics.md`, five phased
...
Command-block **folding** (shipped — `docs/command-blocks.md`): collapse a completed
...
Command-block **annotations** (shipped — `docs/command-blocks.md`): two
...
Structured **history palette** (shipped — `docs/command-blocks.md`): a
...
**Hint mode** (shipped — `docs/configuration.md` `hints`): `prefix f` / `:hints`
...
**Day-1 UX polish** (shipped — seven phases, each
...
(alt|ctrl, shift rejected) with tab reorder — shipped 2026-06-24.)
