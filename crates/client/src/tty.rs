...
impl HostTty {
    ...
    pub fn restore(&mut self) -> Result<(), ClientError> {
        ...
        termios::tcsetattr(fd, SetArg::TCSANOW, &self.original)
            .map_err(|e| ClientError::Tty(e.to_string()))?;
        // Disable kitty keyboard protocol, then mouse tracking, then re-enable
        // cursor and exit alternate screen.
        let mut out = std::io::stdout();
        let _ = out.write_all(b"\x1b[<u\x1b[?1003l\x1b[?1006l\x1b[?25h\x1b[?1049l");
        let _ = out.flush();
        self.restored = true;
        Ok(())
    }
