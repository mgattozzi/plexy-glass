...
fn truncate_is_a_prefix_within_budget(tc: TestCase) {
    ...
    assert!(s.starts_with(t), "truncation must return a prefix of the input");
    assert!(advance_width(t) <= u32::from(max), "truncated advance exceeds the budget");
}
...
fn truncate_is_a_noop_when_the_string_fits(tc: TestCase) {
    ...
    assert_eq!(truncate_to_width(&s, max), s, "must not truncate a string that fits");
}
...
fn grapheme_advance_is_raw_width_clamped_to_one(tc: TestCase) {
    ...
    for (g, adv) in graphemes_with_width(&s) {
        assert_eq!(adv, display_width(g).max(1), "advance must be the raw width clamped to ≥ 1");
...
fn truncate_is_monotonic_in_budget(tc: TestCase) {
    ...
    assert!(large.starts_with(small), "a larger budget keeps at least the smaller prefix");
}
