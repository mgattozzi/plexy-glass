...
/// Which screen buffer a keyboard operation targets. The Kitty flag stacks are
/// independent per buffer, so `Main` and `Alt` pick which stack to touch.
...
impl KeyboardState {
    ...
    pub const fn kitty_set(&mut self, screen: ScreenBuffer, flags: u8, mode: u8) {
        self.active_mut(screen).set(flags, mode);
    }

    /// `\e[><flags>u` pushes onto the active screen's stack.
    pub fn kitty_push(&mut self, screen: ScreenBuffer, flags: u8) {
        self.active_mut(screen).push(flags);
    }

    /// `\e[<<n>u` pops the active screen's stack.
    pub fn kitty_pop(&mut self, screen: ScreenBuffer, n: u16) {
        self.active_mut(screen).pop(n);
    }

    /// Current Kitty flags for the active screen, read by both the `\e[?u`
    /// reply and the daemon's re-encode stage.
    pub const fn kitty_flags(&self, screen: ScreenBuffer) -> u8 {
        self.active(screen).current
    }
    ...
    const fn active(&self, screen: ScreenBuffer) -> &KittyStack {
        match screen {
            ScreenBuffer::Alt => &self.kitty_alt,
            ScreenBuffer::Main => &self.kitty_main,
        }
    }

    const fn active_mut(&mut self, screen: ScreenBuffer) -> &mut KittyStack {
        match screen {
            ScreenBuffer::Alt => &mut self.kitty_alt,
            ScreenBuffer::Main => &mut self.kitty_main,
        }
    }
}
