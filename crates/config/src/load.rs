use crate::{built_in_default, Config};
...
fn config_path() -> Result<PathBuf, ConfigError> {
    let dirs = directories::ProjectDirs::from("", "", "plexy-glass")
        .ok_or(ConfigError::NoHome)?;
    Ok(dirs.config_dir().join("config.toml"))
}

/// Load `~/.config/plexy-glass/config.toml`. Missing file => built-in
/// default, no error. Parse error => built-in default + Some(err).
pub fn load_or_default() -> (Config, Option<ConfigError>) {
    ...
    match toml::from_str::<Config>(&bytes) {
        ...
        Err(e) => {
            ...
            (built_in_default(), Some(ConfigError::Toml(e)))
        }
    }
}
