...
fn fold_projection_visible_unified_round_trip(tc: TestCase) {
    ...
    for v in 0..vt {
        let u = proj.to_unified(v);
        assert!(
            u < total.max(1),
            "to_unified({v})={u} out of range (total {total})"
        );
        assert_eq!(
            proj.from_unified(u),
            Some(v),
    ...
    for v in 0..vt.saturating_sub(1) {
        assert!(
            proj.to_unified(v) < proj.to_unified(v + 1),
...
fn identity_projection_is_a_no_op(tc: TestCase) {
    ...
    for v in 0..n {
        assert_eq!(proj.to_unified(v), v);
        assert_eq!(proj.from_unified(v), Some(v));
    }
}
...
fn scroll_geometry_inverse(tc: TestCase) {
    ...
    for v in 0..vt {
        ...
        let target = proj.to_unified(v);
        let off = blocks::scroll_offset_for_top(&s, rows, target);
        assert!(off <= max, "offset {off} exceeds max {max}");
        assert_eq!(
